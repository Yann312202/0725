...
-(void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    if (searchController.isActive) {
        ...
        if ([searchString length] > 0) {
            ...
            searchResult = [app.arrayName4 filteredArrayUsingPredicate:p];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == self.tableMsg) {//內容頁
        ...
        if (searchResult != nil) {
            ...
            cell.textLabel.text = [app.arrayName4 objectAtIndex:indexPath.row];
            cell.detailTextLabel.text=[app.arrayDialog4 objectAtIndex:indexPath.row] ;
        }
        
        cell.imageView.image= [UIImage imageNamed:[app.arrayImg4 objectAtIndex:(indexPath.row)]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.tableArea) {
        ...
        sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page4 WHERE area ='%@'",cell.textLabel.text];
        if ([cell.textLabel.text isEqualToString:@"所有"]){
            sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page4"];
    ...
    if (tableView == self.tableCity) {
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page4 WHERE city ='%@'",cell.textLabel.text];
            if ([cell.textLabel.text isEqualToString:@"所有"]){
                sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page4"];
                [self.buttonSelect1 setTitle:@"所有縣市" forState:UIControlStateNormal];
            }
    ...
    if(tableView == self.tableTown){
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page4 WHERE town ='%@'",cell.textLabel.text];
            const char *sql = [sqlNsstring cStringUsingEncoding:NSUTF8StringEncoding];
            sqlite3_prepare(db, sql, -1, &statement, NULL);
    ...
    if (tableView == self.tableMsg) {
        ...
        app.pageName=@"page4";//來源
        app.selectImg=[app.arrayImg4 objectAtIndex:indexPath.row];
        app.selectId=[app.arrayId4 objectAtIndex:indexPath.row];
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page4 WHERE name ='%@'",app.selectName];
            const char *sql = [sqlNsstring cStringUsingEncoding:NSUTF8StringEncoding];
            sqlite3_prepare(db, sql, -1, &statement, NULL);
