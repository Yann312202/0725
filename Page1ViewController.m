...
-(void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    if (searchController.isActive) {
        ...
        if ([searchString length] > 0) {
            ...
            searchResult = [app.arrayName1 filteredArrayUsingPredicate:p];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == self.tableMsg) {//內容頁
        ...
        if (searchResult != nil) {
            ...
            cell.textLabel.text = [app.arrayName1 objectAtIndex:indexPath.row];
            cell.detailTextLabel.text=[app.arrayDialog1 objectAtIndex:indexPath.row] ;
        }
        
        cell.imageView.image= [UIImage imageNamed:[app.arrayImg1 objectAtIndex:(indexPath.row)]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.tableArea) {
        ...
        sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page1 WHERE area ='%@'",cell.textLabel.text];
        if ([cell.textLabel.text isEqualToString:@"所有"]){
            sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page1"];
    ...
    if (tableView == self.tableCity) {
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page1 WHERE city ='%@'",cell.textLabel.text];
            if ([cell.textLabel.text isEqualToString:@"所有"]){
                sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page1"];
                [self.buttonSelect1 setTitle:@"所有縣市" forState:UIControlStateNormal];
            }
    ...
    if(tableView == self.tableTown){
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page1 WHERE town ='%@'",cell.textLabel.text];
            const char *sql = [sqlNsstring cStringUsingEncoding:NSUTF8StringEncoding];
            sqlite3_prepare(db, sql, -1, &statement, NULL);
    ...
    if (tableView == self.tableMsg) {
        ...
        app.selectImg=[app.arrayImg1 objectAtIndex:indexPath.row];
        app.selectId=[app.arrayId1 objectAtIndex:indexPath.row];
