...
-(void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    if (searchController.isActive) {
        ...
        if ([searchString length] > 0) {
            ...
            searchResult = [app.arrayName2 filteredArrayUsingPredicate:p];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == self.tableMsg) {//內容頁
        ...
        if (searchResult != nil) {
            ...
            cell.textLabel.text = [app.arrayName2 objectAtIndex:indexPath.row];
            cell.detailTextLabel.text=[app.arrayDialog2 objectAtIndex:indexPath.row] ;
        }
        
        cell.imageView.image= [UIImage imageNamed:[app.arrayImg2 objectAtIndex:(indexPath.row)]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.tableArea) {
        ...
        sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page2 WHERE area ='%@'",cell.textLabel.text];
        if ([cell.textLabel.text isEqualToString:@"所有"]){
            sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page2"];
    ...
    if (tableView == self.tableCity) {
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page2 WHERE city ='%@'",cell.textLabel.text];
            if ([cell.textLabel.text isEqualToString:@"所有"]){
                sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page2"];
                [self.buttonSelect1 setTitle:@"所有縣市" forState:UIControlStateNormal];
            }
    ...
    if(tableView == self.tableTown){
        ...
        if (db != nil) {
            ...
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page2 WHERE town ='%@'",cell.textLabel.text];
            const char *sql = [sqlNsstring cStringUsingEncoding:NSUTF8StringEncoding];
            sqlite3_prepare(db, sql, -1, &statement, NULL);
    ...
    if (tableView == self.tableMsg) {
        ...
        app.pageName=@"page2";//來源
        app.selectImg=[app.arrayImg2 objectAtIndex:indexPath.row];
        app.selectId=[app.arrayId2 objectAtIndex:indexPath.row];
        ...
        if (db != nil) {
            ...
            NSMutableArray *arrayName = [[NSMutableArray alloc]init];
            NSMutableArray *arrayIntroduction = [[NSMutableArray alloc]init];
            NSString *sqlNsstring = [NSString stringWithFormat:@"SELECT * FROM page2 WHERE name ='%@'",app.selectName];
            const char *sql = [sqlNsstring cStringUsingEncoding:NSUTF8StringEncoding];
            sqlite3_prepare(db, sql, -1, &statement, NULL);
